...
export async function getGasData(network: NETWORKS, blockNr?: number) {
  ...
  const blockParams: GetBlockParameters = blockNr ? { blockNumber: BigInt(blockNr) } : { blockTag: "latest" };
  const pendingParams: GetBlockParameters = blockNr ? { blockNumber: BigInt(blockNr + 1) } : { blockTag: "pending" };
  let feeHistoryParams: any = { blockCount: 25, rewardPercentiles: [1, 40, 80] };
    ...
    return {
      ...
      oracle: {
        ...
        normal: {
          ...
          gwei: toRoundedGwei(Math.round(normal + Number(pending.baseFeePerGas))),
        },
        fast: {
...
export function formatFeeHistory(result: any, inclPending = false) {
  ...
  for (let i = 0; i < result.reward.length; i++) {
    blocks.push({
      ...
      priorityFeePerGas: result.reward[i].map((x: any) => Number(x)) as number[],
    });
  }
